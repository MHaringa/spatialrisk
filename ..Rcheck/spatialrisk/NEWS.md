...
* Added `concentration_hotspot()` as the main function for fixed-radius
  ...
  `hotspots` and `contributing_points` components. Legacy components `conc_df`
  and `pts_df` are retained as aliases for compatibility.
* Added `method = "pair_refine"` as the default method for
  `concentration_hotspot()`. It uses terra moving-window screening to select a
  local candidate area and then refines that area with an Rcpp
  pair-intersection search. If the local subset exceeds
  `max_refinement_points`, it falls back to the grid refinement used by
  `method = "terra"`.
* Added `method = "indexed"` to `concentration_hotspot()`, an experimental Rcpp
  spatial-indexing alternative that searches observed point locations only.
