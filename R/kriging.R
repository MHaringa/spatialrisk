#' Ordinary kriging
#'
#' @description Interpolation and smoothing on the sphere by means of ordinary kriging.
#'
...
#' @details Kriging can be considered as linear regression with spatially correlated residuals. Kriging is most appropriate when it is known there is a spatially correlated distance or directional bias in the data. It is often used in soil science and geology.
#' @details  See \code{\link[spatialrisk:interpolate_spline]{splines on the sphere}} for interpolation and smoothing on the sphere by means of splines.
#'
#' @return Object equal to object \code{targets} including extra columns for the predicted value and the variance.
#'
...
interpolate_krige <- function(observations, targets, value, lon_obs = lon, lat_obs = lat, lon_targets = lon, lat_targets = lat) {
  ...
  observation_sf <- sf::st_as_sf(observations, coords = c(lon_obs, lat_obs), crs = 4326)
  targets_sf <- sf::st_as_sf(targets, coords = c(lon_targets, lat_targets), crs = 4326)

  observation_sp <- as(observation_sf, 'Spatial')
  suppressWarnings({
    fit_variogram <- automap::autofitVariogram(f, observation_sp)$var_model
  })
  output <- gstat::krige(formula = f, observation_sf, targets_sf, model = fit_variogram, debug.level = 0)
