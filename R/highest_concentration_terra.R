...
#'   centres in that cell. With the default automatic lower bound, exact scoring
#'   is consequently restricted to observed or pair-intersection centres whose
#'   own raster cell passed screening; the two centres generated by a point pair
#'   are tested separately. Each retained centre is nevertheless scored against
#'   the complete active portfolio, not only the points used to generate it.
#'   This centre-level pruning is disabled for a user-supplied threshold or
#'   negative values. The pair-refined result is exact within the retained
#'   candidate areas under these assumptions. The \code{"observed"} method is
...
hotspot_raster_template <- function(points, cell_size, crs) {
