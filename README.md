...
`points_within_radius()` returns the observations within a given
distance from a specified centre. This is useful for inspecting which
policies, buildings, or other exposure points contribute to a local
aggregate.
...
The output is the target data with an additional column containing the
total value from the reference data within the specified radius.
...
can be used to identify the local portfolio concentration that is
relevant for a regulatory or internal risk limit. By default,
`method = "continuous"` searches for a centre that may lie between
buildings. Internally, it uses a coarse spatial screening step followed
by local pair-intersection refinement.
